  ...
  <div style={{ display:"flex", gap:6, alignItems:"center" }}>
    {[1,2,3].map(n => (
      <div key={n} style={{ display:"flex", alignItems:"center", gap:6 }}>
        <div style={{ width:n<=current?10:7, height:n<=current?10:7, borderRadius:999, transition:"all .3s",
          background:n<=current?color:"rgba(255,255,255,0.1)",
          boxShadow:n<=current?`0 0 7px ${color}99`:"none",
          border:n===current?`2px solid ${color}`:"none" }}/>
      ...
      <div style={{ maxWidth:600, margin:"0 auto", padding:"32px 20px", display:"flex", flexDirection:"column", gap:20 }}>
        ...
        <div style={{ background:"rgba(232,179,75,0.06)", borderRadius:14, border:"1px solid rgba(232,179,75,0.2)", padding:"14px 18px", display:"flex", gap:10 }}>
          ...
          <div style={{ fontSize:13, color:"rgba(255,255,255,0.5)", lineHeight:1.7 }}>
            Your donation is held in <strong style={{ color:"#e8b34b" }}>secure escrow</strong> and only released to the missionary when milestone proof is verified by our admin team.
          </div>
        </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:8 }}>
            {AMOUNTS.map(a => (
              <button key={a} onClick={() => setAmt(String(a))}
                style={{ padding:"11px 0", borderRadius:12, border:`1px solid ${amt===String(a)?m.color:"rgba(255,255,255,0.1)"}`, background:amt===String(a)?`${m.color}22`:"rgba(255,255,255,0.03)", color:amt===String(a)?m.color:"rgba(255,255,255,0.5)", fontWeight:700, cursor:"pointer", fontSize:14, transition:"all .15s" }}>
                ${a}
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <div style={{ display:"flex", borderRadius:13, overflow:"hidden", border:`1px solid ${amt&&!AMOUNTS.includes(Number(amt))?m.color:"rgba(255,255,255,0.1)"}` }}>
            ...
            <input type="number" value={AMOUNTS.includes(Number(amt))?"":amt} onChange={e=>setAmt(e.target.value)} placeholder="Enter amount"
              style={{ flex:1, padding:"13px 15px", background:"rgba(255,255,255,0.03)", border:"none", color:"#eef1ff", fontSize:18, fontFamily:"Georgia, serif", outline:"none" }}/>
          </div>
        </div>

        {/* Donate button */}
        <button onClick={() => { if(amt && Number(amt) > 0) onSuccess(amt); }}
          style={{ padding:"16px 0", borderRadius:14, border:"none", background:amt&&Number(amt)>0?`linear-gradient(135deg,${m.color},${m.color}cc)`:"rgba(255,255,255,0.06)", color:amt&&Number(amt)>0?"#000":"rgba(255,255,255,0.25)", fontWeight:700, cursor:amt&&Number(amt)>0?"pointer":"default", fontSize:16, fontFamily:"Georgia, serif", letterSpacing:.3, boxShadow:amt&&Number(amt)>0?`0 6px 28px ${m.color}44`:"none", transition:"all .2s" }}>
        ...
        <div style={{ textAlign:"center", fontSize:12, color:"rgba(255,255,255,0.2)" }}>
          🔒 Funds held in escrow · Released only on verified proof of work
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ textAlign:"center", maxWidth:480, display:"flex", flexDirection:"column", gap:20, alignItems:"center" }}>
      ...
      <div>
        ...
        <div style={{ fontSize:15, color:"rgba(255,255,255,0.5)", lineHeight:1.8 }}>
          Your gift of <strong style={{ color:"#3ecf8e" }}>${amt}</strong> to <strong style={{ color:"#eef1ff" }}>{m.protected ? "this protected missionary" : m.name}</strong>'s mission has been placed in escrow.<br/>
          You will receive a notification when the next milestone proof is submitted.
        </div>
      </div>
      ...
      <button onClick={onContinue}
        style={{ padding:"14px 40px", borderRadius:14, border:"none", background:"linear-gradient(135deg,#3ecf8e,#2aaf74)", color:"#000", fontWeight:700, cursor:"pointer", fontSize:15, fontFamily:"Georgia, serif", boxShadow:"0 6px 28px rgba(62,207,142,0.4)" }}>
        ← Continue Exploring
      </button>
    </div>
  </div>
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#060c18", color:"#eef1ff", fontFamily:"Georgia, serif" }}>
      ...
      <div style={{ background:"#09111f", borderBottom:"1px solid rgba(255,255,255,0.07)", padding:"16px 24px", display:"flex", alignItems:"center", gap:14, position:"sticky", top:0, zIndex:100 }}>
        ...
        <span style={{ marginLeft:"auto", padding:"4px 12px", borderRadius:999, fontSize:12, background:m.status==="complete"?"rgba(62,207,142,0.15)":`${m.color}18`, color:m.status==="complete"?"#3ecf8e":m.color, border:`1px solid ${m.status==="complete"?"#3ecf8e33":m.color+"33"}` }}>
          {m.status==="complete" ? "✓ Completed" : m.protected ? "🔒 Protected" : "● Active"}
        </span>
      </div>
      ...
      <div style={{ maxWidth:700, margin:"0 auto", padding:"0 20px 60px" }}>
          ...
          <div style={{ display:"flex", gap:16, alignItems:"flex-start", marginBottom:16 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:13, color:"rgba(255,255,255,0.35)", marginTop:4 }}>
                📍 {m.protected ? `${m.region} (Location Protected)` : `${m.area}, ${m.city}, ${m.country}`}
              </div>
            </div>
          </div>
        ...
        <div style={{ padding:"24px 0", borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              {["photos","videos"].map(t=>(
                <button key={t} onClick={()=>setProofTab(t)}
                  style={{ padding:"5px 14px", borderRadius:8, border:`1px solid ${proofTab===t?m.color:"rgba(255,255,255,0.1)"}`, background:proofTab===t?`${m.color}18`:"transparent", color:proofTab===t?m.color:"rgba(255,255,255,0.4)", cursor:"pointer", fontSize:12, fontFamily:"Georgia, serif" }}>
        ...
        <div style={{ padding:"20px 0 0" }}>
          ...
          <button onClick={onDonate}
            style={{ width:"100%", padding:"16px 0", borderRadius:14, border:"none", background:`linear-gradient(135deg,${m.color},${m.color}cc)`, color:"#000", fontWeight:700, cursor:"pointer", fontSize:16, fontFamily:"Georgia, serif", letterSpacing:.3, boxShadow:`0 6px 28px ${m.color}44` }}>
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"24px 20px" }}>
        ...
        <div style={{ borderRadius:20, overflow:"hidden", marginBottom:24, border:"1px solid rgba(255,255,255,0.08)", position:"relative", height:280 }}>
          ...
          {MISSIONS.map(m => {
            ...
            return (
              <div key={m.id} onClick={() => onMission(m)} title={m.protected ? "Protected Mission" : m.name}
        ...
        <div style={{ display:"flex", gap:8, marginBottom:20, flexWrap:"wrap" }}>
          {REGIONS.map(r => (
            <button key={r} onClick={() => setRegion(r)}
              style={{ padding:"7px 16px", borderRadius:999, border:`1px solid ${region===r?"#e8b34b":"rgba(255,255,255,0.1)"}`, background:region===r?"rgba(232,179,75,0.15)":"rgba(255,255,255,0.03)", color:region===r?"#e8b34b":"rgba(255,255,255,0.5)", cursor:"pointer", fontSize:13, fontWeight:600, transition:"all .15s" }}>{r}</button>
          ))}
        </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          {visible.map(m => (
            <div key={m.id} onClick={() => onMission(m)}
              style={{ background:"#0c1628", borderRadius:18, border:`1px solid ${m.color}22`, padding:20, cursor:"pointer", borderLeft:`3px solid ${m.color}`, transition:"transform .15s, box-shadow .15s" }}
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                ...
                <span style={{ padding:"3px 10px", borderRadius:999, fontSize:11, background:m.status==="complete"?"rgba(62,207,142,0.15)":`${m.color}18`, color:m.status==="complete"?"#3ecf8e":m.color, border:`1px solid ${m.status==="complete"?"#3ecf8e33":m.color+"33"}`, whiteSpace:"nowrap" }}>
              ...
              <button onClick={e => { e.stopPropagation(); onMission(m); }}
                style={{ width:"100%", padding:"12px 0", borderRadius:12, border:"none", background:`linear-gradient(135deg,${m.color},${m.color}cc)`, color:"#000", fontWeight:700, cursor:"pointer", fontSize:14, boxShadow:`0 4px 20px ${m.color}44` }}>
                View Mission & Donate →
              </button>
            </div>
          ))}
...
export default function App() {
  ...
  return <HomeScreen onMission={openMission}/>;
}
