...
const SHARE_TYPES = [
  { key:"app",      label:"SendMe App",          desc:"Share the SendMe platform with your church",         url:"https://sendmeglobalmission.org" },
  { key:"missions", label:"View All Missions",   desc:"Direct link to the active missions list",            url:"https://sendmeglobalmission.org/missions" },
...
export default function QRShare({ missions = [], onBack }) {
  ...
  const currentUrl = selectedMission
    ? `https://sendmeglobalmission.org/mission/${selectedMission.id}`
    : SHARE_TYPES.find(t => t.key === activeType)?.url || "https://sendmeglobalmission.org";
