...
const DEMO_REQUESTS = [
  { id:1, title:"Missionary team needed — Northern Mozambique", church:"Maputo Tabernacle", country:"Mozambique", region:"Africa", type:"missionary_team", description:"We have established contacts in 3 villages in Northern Mozambique who are hungry for the Message. We need a missionary team to come and hold cottage meetings. We can host them.", needs:["2 missionaries","Transport support","Accommodation provided"], responses:1, created_at:"2025-05-10T08:00:00Z" },
  { id:2, title:"Bible teacher needed — Rural Zimbabwe",         church:"Harare End Time Fellowship", country:"Zimbabwe", region:"Africa", type:"bible_teacher", description:"We have a group of 15 new believers in a rural area 3 hours from Harare. They need weekly Bible teaching. Looking for someone who can commit to monthly visits.", needs:["1 Bible teacher","Monthly visits","Transport support needed"], responses:0, created_at:"2025-05-08T10:00:00Z" },
  { id:3, title:"Evangelism support — Dalit villages, India",   church:"Chennai New Life Church", country:"India", region:"Asia", type:"evangelist", description:"Sister Priya's literacy mission has opened doors in 5 more Dalit villages. We urgently need evangelists who can follow up on contacts and hold meetings.", needs:["2 evangelists","Tamil/Telugu speakers preferred","6 month commitment"], responses:2, created_at:"2025-05-06T14:00:00Z" },
];
...
export default function SendAWorker({ onBack, user }) {
  ...
  const [form, setForm]             = useState({ title:"", description:"", church:"", country:"", type:"missionary_team", need1:"", need2:"", need3:"" });
  const [response, setResponse]     = useState({ commitment:"", canHost:false, transport:false, note:"" });
  ...
  const submitRequest = async () => {
    ...
    try {
      await supabase.from("worker_requests").insert({ ...form, needs:[form.need1,form.need2,form.need3].filter(Boolean), responses:0, created_at:new Date().toISOString() });
    } catch {}
  ...
  const submitResponse = async (req) => {
    try { await supabase.from("worker_responses").insert({ request_id:req.id, ...response, user_id:user?.id }); } catch {}
          ...
          {responded ? (
            <div style={{ textAlign:"center", padding:"40px 0" }}>
              ...
              <div style={{ fontSize:14, color:"rgba(255,255,255,0.5)", lineHeight:1.8, marginBottom:20 }}>
                <strong style={{ color:"#e8b34b" }}>{req.church}</strong> in {req.country} will be notified of your response. They will contact you directly to coordinate.
              </div>
            ...
            <>
              <div style={{ background:`${t.color}12`, borderRadius:14, border:`1px solid ${t.color}33`, padding:18, marginBottom:20 }}>
                ...
                <div style={{ fontSize:13, color:"rgba(255,255,255,0.4)" }}>{req.church} · {req.country}</div>
              </div>
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#060c18", color:"#eef1ff", fontFamily:"Georgia, serif" }}>
      <div style={{ background:"#09111f", borderBottom:"1px solid rgba(255,255,255,0.07)", padding:"16px 24px", display:"flex", alignItems:"center", gap:14, position:"sticky", top:0, zIndex:100 }}>
        ...
        <button onClick={()=>setShowForm(f=>!f)} style={{ marginLeft:"auto", background:"linear-gradient(135deg,#e8b34b,#c8942b)", border:"none", borderRadius:10, padding:"8px 16px", color:"#000", cursor:"pointer", fontSize:13, fontWeight:700 }}>
          + Post a Need
        </button>
      </div>
          ...
          <div style={{ background:"#0c1628", borderRadius:16, border:"1px solid rgba(232,179,75,0.25)", padding:20, marginBottom:24 }}>
            {submitted ? (
              <div style={{ textAlign:"center", padding:"20px 0" }}>
                ...
                <div style={{ fontSize:16, fontWeight:700, color:"#eef1ff", marginBottom:8 }}>Request Posted</div>
                <div style={{ fontSize:13, color:"rgba(255,255,255,0.5)" }}>Other churches can now see and respond to your need.</div>
              </div>
            ) : (
              <>
            ...
            return (
              <div key={req.id} style={{ background:"#0c1628", borderRadius:16, border:`1px solid ${t.color}33`, borderLeft:`3px solid ${t.color}`, padding:20 }}>
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  ...
                  <button onClick={()=>{setResponding(req);setResponded(false);setResponse({commitment:"",note:"",canHost:false,transport:false});}}
