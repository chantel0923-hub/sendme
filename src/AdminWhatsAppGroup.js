...
export default function AdminWhatsAppGroup({ onBack }) {
  ...
  const copyAll = () => {
    const text = filtered.map(r => `${r.full_name || "Unnamed"} — ${r.whatsapp_number || "no number"}`).join("\n");
    ...
    <div style={{ minHeight: "100vh", background: "#060c18", color: "#eef1ff", fontFamily: "Georgia, serif" }}>
      <div style={{ background: "#09111f", borderBottom: "1px solid rgba(255,255,255,0.07)", padding: "16px 24px", display: "flex", alignItems: "center", gap: 14, position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", letterSpacing: 2, marginTop: 2 }}>{rows.length} OPTED IN — ADD MANUALLY TO YOUR GROUP</div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "24px 20px 60px" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
          ...
          <button onClick={copyAll} disabled={filtered.length === 0}
            style={{ padding: "11px 18px", borderRadius: 10, border: "1px solid rgba(37,211,102,0.35)", background: "rgba(37,211,102,0.1)", color: copied ? "#3ecf8e" : "#25d366", fontWeight: 700, cursor: filtered.length === 0 ? "default" : "pointer", fontSize: 13, fontFamily: "Georgia, serif", whiteSpace: "nowrap" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {filtered.map(r => (
            <div key={r.id} style={{ background: "#0c1628", borderRadius: 12, border: "1px solid rgba(255,255,255,0.07)", padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ fontSize: 14, color: "#25d366", fontWeight: 700, whiteSpace: "nowrap" }}>{r.whatsapp_number || "—"}</div>
            </div>
          ))}
        </div>
      </div>
